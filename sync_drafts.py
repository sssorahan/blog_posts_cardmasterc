...
def main():
    ...
    print(f"[INFO] 이미 추적 중인 post_id: {len(tracked_ids)}개")

    # Blogger 초안 전체 조회 (fields 없이 전체 응답)
    ...
    while True:
        ...
        draft_posts.extend(items)
        print(f"  → {len(items)}개 조회됨 (누적 {len(draft_posts)}개)")
        page_token = resp.get('nextPageToken')
        if not page_token:
            break

    print(f"[INFO] Blogger 초안 총 {len(draft_posts)}개 확인됨")
    ...
    for html_file in sorted(BASE_DIR.glob('*.html')):
        ...
        if m:
            html_title_map[m.group(1).strip()] = html_file.name
    ...
    for post in draft_posts:
        ...
        if not filename:
            print(f"  [WARN] 매칭 HTML 없음: {title[:60]}")
            continue
        ...
        if filename not in pending:
            ...
            added_pending += 1
            print(f"  [OK] pending 추가: {title[:60]}")
    ...
    print(f"\n[완료] uploaded +{added_uploaded}개 / pending +{added_pending}개")
    print(f"       발행 대기: {len(pending)}개")
